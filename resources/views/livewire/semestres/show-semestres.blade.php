            ...
            <div style="max-width:700px; margin: 30px auto;">
                <div class="flex items-center">
                    <div class="relative w-full mt-6">
                        ...
                        <input type="text" id="voice-search" class="rounded-lg bg-gray-50 border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600"  placeholder="Buscar Por Semestre" wire:model="search" >
                    </div>


                    <div class="mt-6">
                        @livewire('semestres.crear-semestre')
                    </div>
                                                ...
                                                @foreach($semestres as $semestre)
                                                    <tr class="border-b border-gray-200 bg-gray-100">
                                                        <td class="py-3 px-6 text-center whitespace-nowrap">
                                                            <div class="flex items-center">
                                                            <span
                                                                class="font-medium uppercase">{{ $semestre->id}}</span>
                                                            </div>
                                                        </td>
                                                        <td class="py-3 px-6 text-center whitespace-nowrap">
                                                            <div class="flex items-center">
                                                                <span class="font-medium uppercase">{{$semestre->nombre}}</span>
                                                            </div>
                                                        </td>
                                                        ...
                                                        <td class="py-3 px-6 text-center">
                                                            <div class="flex item-center justify-center">
                                                                <div class="mr-2">
                                                                    @livewire('semestres.editar-semestre',['semestre'=>$semestre],key($semestre->id))
                                                                </div>
                                                                <div
                                                                class=" mr-2  transform hover:text-purple-500 hover:scale-110 uppercase">
                                                                <a onclick="confirm('Estas Seguro de Eliminar A {{$semestre->nombres}}!')||event.stopImmediatePropagation()"
                                                                   wire:click="destroy({{$semestre->id}})">
                                                                    <button
